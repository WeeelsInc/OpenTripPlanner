 ...
 */
public class CanadaWayPropertySetSource implements WayPropertySetSource {
   ...
   public WayPropertySet getWayPropertySet() {
      ...
      setProperties(props, "highway=byway", StreetTraversalPermission.ALL, 1.3,
            1.3);
      setProperties(props, "highway=track", StreetTraversalPermission.ALL, 1.3,
            1.3);
      ...
      setProperties(props, "highway=tertiary_link",
            StreetTraversalPermission.ALL, 1, 1);
      setProperties(props, "highway=secondary", StreetTraversalPermission.ALL,
            1.5, 1.5);
      ...
      // connections
      setProperties(props, "highway=trunk_link", StreetTraversalPermission.CAR,
            2.06, 2.06);
      ...
      /* Automobile speeds in the Canada */
      setCarSpeed(props, "highway=motorway", 27.8f); // 27.8 m/s ~= 100 kph
      setCarSpeed(props, "highway=motorway_link", 16.7f); // ~= 60 kph
      setCarSpeed(props, "highway=trunk", 24.6f); // ~= 55 mph
      setCarSpeed(props, "highway=trunk_link", 15f); // ~= 35 mph
      setCarSpeed(props, "highway=primary", 23.6f); // ~= 85 kph
      setCarSpeed(props, "highway=primary_link", 11.1f); // ~= 40 kph
      setCarSpeed(props, "highway=secondary", 16.7f); // ~= 60 kph
      setCarSpeed(props, "highway=secondary_link", 11.1f); // ~= 40 kph
      setCarSpeed(props, "highway=tertiary", 13.9f); // ~= 50 kph
      setCarSpeed(props, "highway=tertiary_link", 11.1f); // ~= 40 kph
      setCarSpeed(props, "highway=living_street", 2.2f); //
      setCarSpeed(props, "highway=residential", 11.1f); // ~= 40 kph
      setCarSpeed(props, "highway=unclassified", 11.1f); // ~= 40 kph
      setCarSpeed(props, "highway=service", 6.7f); //
      setCarSpeed(props, "highway=track", 4.5f); //
      setCarSpeed(props, "highway=road", 12.5f); // ~= 45 kph
      ...
      /*
       * The following tags have been removed from surface weights because they
       * are no more of an impedence to bicycling than a paved surface
       * surface=paving_stones surface=fine_gravel (sounds counter-intuitive but
       * see the definition on the OSM Wiki) surface=tartan (this what running
       * tracks are usually made of)
       */

      setProperties(props, "surface=unpaved", StreetTraversalPermission.ALL,
            1.18, 1.18, true);
      setProperties(props, "surface=compacted", StreetTraversalPermission.ALL,
            1.18, 1.18, true);
      setProperties(props, "surface=wood", StreetTraversalPermission.ALL, 1.18,
            1.18, true);
      ...
      setProperties(props, "surface=metal", StreetTraversalPermission.ALL, 1.3,
            1.3, true);
      setProperties(props, "surface=ground", StreetTraversalPermission.ALL,
            1.5, 1.5, true);
      setProperties(props, "surface=dirt", StreetTraversalPermission.ALL, 1.5,
            1.5, true);
      setProperties(props, "surface=earth", StreetTraversalPermission.ALL, 1.5,
            1.5, true);
      setProperties(props, "surface=grass", StreetTraversalPermission.ALL, 1.5,
            1.5, true);
       ...
       * above. This switch makes our weighting system less reliant on tags that
       * aren't generally used by the OSM community, and prevents the double
       * counting that was occuring on streets with both bicycle infrastructure
       * and an RLIS:bicycle=designated tag
       */
